...
class WJSBaseballFunctionLibrary
{
public:
	static void MyPrintString(
		const AActor* InWorldContextActor,
	...
	static FString GetRoleString(const AActor* InActor)
	{
		...
		{
			const FString LocalRoleString = UEnum::GetValueAsString(
				TEXT("Engine.ENetRole"), InActor->GetLocalRole());
			const FString RemoteRoleString = UEnum::GetValueAsString(
				TEXT("Engine.ENetRole"), InActor->GetRemoteRole());

			RoleString = FString::Printf(
				TEXT("%s / %s"), *LocalRoleString, *RemoteRoleString);
		}
